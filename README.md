...
1. **Base Case Check**: This is crucial to prevent infinite recursion. It defines a condition that, when met, stops the recursion. Without a base case, the recursion would continue indefinitely, leading to a stack overflow.
...
3. **Output**: The recursive method performs some useful work or computation in each recursive call. In this case, it prints the current number. Recursive methods can perform any desired operation at each step.
...
5. **Incremental Step**: This step modifies the input arguments or state in some way to make progress toward the base case. It ensures that the recursion moves closer to the termination condition, preventing the recursion from becoming stuck.
...
6. **Recursive Call**: The recursive method calls itself with modified arguments. This is the essence of recursion, as it allows the function to solve smaller instances of the same problem. The recursive call typically operates on a smaller or simpler subproblem.
...
8. **Base Case Exit**: This defines the condition under which the recursion stops. When the base case is reached, the recursion naturally terminates. Without a base case exit, the recursion would continue indefinitely.
...
2. Call the method with an argument of 5
3. Run
...
1. Comment out the Console.WriteLine() after our if statement.
2. Now AFTER our recursive call, add the following line.
	`Console.WriteLine($"After Recursive Call: {currentNumber}");`
3. Run The Code with an argument of 5
...
|----------|-------------|--------|
| **Project Setup** | Correct setup of the console application with the specified naming convention. | 20 |
| **Understanding Recursion** | Clear and concise explanation of recursion and the importance of the base case. | 15 |
| **Implementing Recursion** | Correct implementation of the four recursive methods with appropriate comments. | 15 |
| **Explanation of Algorithm Logic** | Detailed explanation of the logic behind recursion, including base case, recursive calls, and call stack behavior. | 15 |
| **Optimizing Recursive Processes** | Implementation and explanation of optimizations in recursive methods. | 15 |
| **Code Testing and Output Accuracy** | Accurate testing and demonstration of all recursive methods with correct output. | 5 |
| **Submission** | Successful upload to GitHub with the correct repository link provided. | 5 |
| **Total** |  | 100 |
